...
for i = 1:2
    ...
    if strcmp('S', first)
        countdown('Now its time to restudy some pairs from the last list', 3, constants.countdownSpeed,  window, constants);
        data.onset(studyRows) = study(data(studyRows, {'cue','target'}), window, constants);
        countdown('It''s time for a practice test on some pairs from the last list', 3, constants.countdownSpeed,  window, constants);    
